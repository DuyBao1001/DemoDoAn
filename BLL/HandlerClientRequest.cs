                ...
                case MessageProtocol.RequestCommand.FORGOT_PASSWORD:
                    {
                        try
                        {
                            ...
                            // Kiểm tra xem payload có phải là định dạng JSON Object không
                            if (rawPayload.Trim().StartsWith("{"))
                            {
                                // Nếu là JSON Object {"Email":"..."}
                            ...
                            {
                                // Nếu là chuỗi đơn "abc@gmail.com" hoặc abc@gmail.com
                                emailFromClient = rawPayload.Trim().Replace("\"", "");
                            }

                            // [QUAN TRỌNG] In ra Console để "bắt tận tay" giá trị gửi xuống
                            ...
                            {
                                // Nếu vào đây nghĩa là SQL không tìm thấy email khớp hoàn toàn
                ...
                case MessageProtocol.RequestCommand.RESET_PASSWORD:
                    {
                        try
                        {
                            ...
                            if (OtpManager.VerifyOtp(resetData.Email, resetData.OTP))
                            {
                                ...
                                // Lưu ý: Thay HashHelper bằng class mã hóa thực tế của bạn
                                string hashedPassword = resetData.NewPassword; // Nhớ Hash trước khi lưu
                                _accountRepo.UpdatePassword(resetData.Email, hashedPassword);
                ...
                case MessageProtocol.RequestCommand.REGISTER:
                    {
                        ...
                        if (regModel != null)
                        {
                            ...
                            // 2. MỚI: Kiểm tra Email đã tồn tại chưa
        ...
        private void SendResponse(string command, string payload)
        {
            if (isConnected && _client.Connected)
            {
                try
                {
                    ...
                    _streamWriter.WriteLine(jsonString); // Gửi chuỗi JSON
                    _streamWriter.Flush(); // Đẩy đi ngay lập tức
                }
                catch (Exception ex)
                {
